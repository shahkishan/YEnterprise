	...
	<div>
		...
		>
			<template slot="items" slot-scope="props">
				...
				<td v-on:click="infoDisplay(props.item)">{{ props.item.state }}</td>
	  ...
	  <v-card tile>
           ...
           <v-subheader>{{dialogItem.add1}}, {{dialogItem.add2}}</v-subheader>
		   <v-divider></v-divider>
		   <v-subheader><h3>Pincode</h3></v-subheader>
           <v-subheader>{{dialogItem.PIN}} </v-subheader>
           <v-divider></v-divider>
		   <v-subheader><h3>State</h3></v-subheader>
           <v-subheader>{{dialogItem.state}} </v-subheader>
           <v-divider></v-divider>
		   	<v-subheader><h3>StateCode</h3></v-subheader>
	...
	export default {
      ...
      v => /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(v) || 'E-mail must be valid'
			...
			headers:[
				...
				{text:'Addressline 1',value:'add1', sortable: false},
				{text:'Addressline 2',value:'add2', sortable: false},
				{text:'Pincode',value:'PIN', sortable: false},
		...
		methods: {
			...
			deleteItem (item) {
				this.$store.dispatch('deleteCompany',item)
